...
Component({
  ...
  didMount() {
    if (!this.props.canvasId) {
      this.setData({ canvasId: `Canvas_${Date.now()}_${Math.ceil(Math.random() * 999999)}` })
    }
  ...
  methods: {
    onCanvasReady() {
      my.createCanvas({
        id: this.props.canvasId || this.data.canvasId,
        success: (canvas) => {
    ...
    initGameApplication(canvas, options = {}) {
      ...
      const application = this.app = new PIXI.Application(obj);
